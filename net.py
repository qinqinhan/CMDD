...
class FCG(nn.Module):
    ...
    def __init__(self, gnn_model_type, num_layers, groups, pool_ratio, kern_size,
        ...
        assert num_layers >= 1, 'Number of layers is invalid.'
        assert num_layers == len(kern_size), 'Number of kernel_size should equal to number of layers.'
    ...
    def forward(self, inputs: Tensor, acc=True):

        B, C, M, L = inputs.size()
        inputs_reshaped = inputs.reshape(B, C, M * L, -1)

        if inputs_reshaped.size(-1) % self.num_graphs:
            pad_size = (self.num_graphs - inputs_reshaped.size(-1) % self.num_graphs) / 2
            x = F.pad(inputs_reshaped, (int(pad_size), math.ceil(pad_size)), mode='constant', value=0.0)
        else:
            x = inputs_reshaped
